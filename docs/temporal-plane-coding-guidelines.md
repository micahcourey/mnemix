# Temporal Plane Coding Guidelines
...
Temporal Plane is a **Rust-first, library-first, local-first** system.

That means:

- Rust is the source of truth for product behavior.
- Python is a binding layer, not a second implementation.
- Adapters translate host concepts into Temporal Plane concepts; they do not redefine product semantics.
...
`temporal-plane-core` must not expose or depend on:
...
- Put shared helpers in `temporal-plane-test-support`.
...
- Core traits speak in Temporal Plane vocabulary.
...
- Does this leak backend details into `temporal-plane-core`?
