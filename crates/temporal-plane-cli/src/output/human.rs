...
mod tests {
    ...
    fn demo_memory_detail() -> MemoryDetailView {
        MemoryDetailView {
            ...
            scope_id: "repo:temporal-plane".to_owned(),
            kind: "decision",
            ...
            source_ref: Some("docs/temporal-plane-roadmap.md".to_owned()),
            tags: vec!["milestone-3".to_owned(), "cli".to_owned()],
            entities: vec!["TemporalPlane".to_owned()],
    ...
    fn status_output_snapshot() {
        let output = CommandOutput::Status(Box::new(StatusView {
            command: "init",
            status: "ok",
            message: "Initialized Temporal Plane store".to_owned(),
...
init: ok
Initialized Temporal Plane store
path: /tmp/plane
schema_version: 1
");
    }
...
source_tool: copilot
source_ref: docs/temporal-plane-roadmap.md
